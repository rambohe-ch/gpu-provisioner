...
var KaitoNodeClaimPredicate = predicate.Or(selector1, selector2)
...
func AllNodesForNodeClaim(ctx context.Context, c client.Client, nodeClaim *v1.NodeClaim) ([]*corev1.Node, error) {
	...
	if nodeClaim.Status.ProviderID == "" {
		...
		if cond != nil && cond.IsFalse() && (cond.Message == "all requested instance types were unavailable during launch" || strings.Contains(cond.Message, "is not allowed in your subscription in location")) {
			return nil, nil // Not recoverable, does not consider as an error
		} else {
			return nil, fmt.Errorf("the nodeClaim(%s) has not been associated with any node yet", nodeClaim.Name)
		}
	}
